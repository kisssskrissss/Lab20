...
namespace Lab20___WpfApp.ViewModels
{
    class MainWindowViewModel : INotifyPropertyChanged
    {
        ...
        //Команда выбора операций нужна для формирования расчетной цифры (number1) и определения дальнейшей операции вычисления
        public ICommand OperButtonCommand { get; }
        private void OnOperButtonCommandExecute(object p)
        {
            //Проверка наличия данных в переменных, если да, промежуточный расчет   
            string dataStr = StrData;
            StrData = "0";
            double tempNumder = 0;
            if (number1 != 0)
        ...
        private void OnMonoButtonCommandExecute(object p)
        {
            ...
            //поскольку моно операции используются только для текущего рассчета, то переводим во временную переменную
            double tempNum = 0;
        ...
        private void OnNegativeButtonCommandExecute(object p)
        {
            ...
            // то резудьтат ее  выполнения выводим в StrData (нижний TexBox)
            double num = 0;
